                    ...
                    if (OWM_TYPE_BBOX.equals(request.getType())) {
                        for (Object obj : body.getJsonArray("list")) {
                            DataSendRequest dataSendRequest =
                                    new DataSendRequest(request.getPipeId(), request.getHopsProjectId(), request.getHopsDataset(), DataType.OWM, "owm_weather", (String) obj, request.getUser(), request.getPassword(), "{}", null);
                            if (aggregate)
                        ...
                        DataSendRequest dataSendRequest =
                                new DataSendRequest(request.getPipeId(), request.getHopsProjectId(), request.getHopsDataset(), DataType.OWM, "owm_weather", body.toString(), request.getUser(), request.getPassword(), "{}", null);
                        if (aggregate)
                                dataSendRequest.setAggregate(true);
                        vertx.eventBus().send(Constants.MSG_SEND_DATA, Json.encode(dataSendRequest));
                    }
